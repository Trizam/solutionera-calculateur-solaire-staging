...
| `webi` | **Uniquement `#sec-prod` + `#sec-prod-b`** — 1A taille (PV + kWc) puis 1B production (kWh/an + kWh/j déc). Coût solaire, valeur/KPI, réserve, batterie, total projet, PDF, hero, mentions masqués |
...
- **masqués en `webi` :** hero, `#sec-cost`, `#sec-value` (KPI + PDF), `#sec-reserve`, `#sec-battery`, `#sec-project`, `aside.disclaimers`, modales info/tarif
