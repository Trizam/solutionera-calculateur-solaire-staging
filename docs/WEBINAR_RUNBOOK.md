...
| `webi` | **Uniquement `#sec-prod` + `#sec-prod-b`** — 1A taille (PV + kWc) puis 1B production (productible kWh/kWc, puis kWh/an + kWh/j déc). Coût, valeur/KPI, PDF, hero, mentions masqués |
...
- **visible en `webi` :** marque + `#sec-prod` (1A : superficie, densité → qté PV + kWc) + `#sec-prod-b` (1B : localisation QC, orientation, inclinaison, déneige → productible kWh/kWc, puis Mesurage Net kWh/an + Autonomie kWh/j déc) + pied de bug
...
| Tarif marginal | **12,811 ¢/kWh** (Tarif D 2<sup>e</sup> tranche **TTC**, 1<sup>er</sup> avr. 2026 ; 11,142 × 1,14975 = 12,8105115, arrondi) |
...
4. Bug report : phrase entière « Signale un bug, une erreur de calcul ou une amélioration » dans le footer (visible aussi en webi) → modale. POST vers `meta[name="bug-report-endpoint"]` (pas de token dans la page). Voir `docs/BUG_REPORTS.md`.
