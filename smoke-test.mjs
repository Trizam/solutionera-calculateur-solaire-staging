...
const hasEcoNote =
  ...
  html.includes("surplus n’est pas crédité") &&
  html.includes("id=\"surplusKwh\"") &&
  html.includes("id=\"surplusLost\"");
const hasCreditFn =
...
const surplusClampMath = (function () {
  ...
  const lost = surplus * rate;
  return surplus === 5000 && Math.abs(lost - 640.55) < 0.01;
})();
const hasRateInfoUi =
...
console.log(`  surplus alert CSS + cartes rouges: ${hasSurplusAlertCss ? "PASS" : "FAIL"}`);
console.log(`  surplus kWh × tarif = $ perdus: ${surplusClampMath ? "PASS" : "FAIL"}`);
